...
final class BasePumpHistoryStorage: PumpHistoryStorage, Injectable {
    ...
    private func applyInsulinSnapshot(to event: PumpEventStored, insulinType: InsulinType?) {
        ...
        event.actionDuration = settingsManager.pumpSettings.insulinActionCurve as NSDecimalNumber
        ...
        return try await context.perform {
            ...
            for event in events {
                ...
                case .bolus:
                    ...
                    // U100-adjusted amount (insulin concentration feature) already rounds to the bolus increment
                    newBolusEntry.amount = NSDecimalNumber(
                        decimal: self.adjustPumpedVolumeToU100(pumpedVolume: dose.unitsInDeliverableIncrements)
                    )
                    newBolusEntry.programmedAmount = NSDecimalNumber(
                        decimal: self.adjustPumpedVolumeToU100(pumpedVolume: dose.programmedUnits)
                    )
                ...
                case .tempBasal:
                    ...
                    newTempBasal.rate = self.adjustPumpedRateToU100(pumpedRate: Decimal(dose.unitsPerHour)) as NSDecimalNumber
                    newTempBasal.startDate = span.start
                    newTempBasal.endDate = span.end
                    newTempBasal.deliveredUnits = dose.deliveredUnits
                        .map { self.adjustPumpedVolumeToU100(pumpedVolume: $0) as NSDecimalNumber }
    ...
    private func updateMutablePumpEvent(_ event: PumpEventStored, with dose: DoseEntry) {
        ...
        case PumpEventStored.EventType.bolus.rawValue:
            ...
            let finalAmount = dose.deliveredUnits.map {
                self.adjustPumpedVolumeToU100(pumpedVolume: $0)
            }
        ...
        case PumpEventStored.EventType.tempBasal.rawValue:
            ...
            tempBasal.rate = adjustPumpedRateToU100(pumpedRate: Decimal(dose.unitsPerHour)) as NSDecimalNumber
            tempBasal.startDate = span.start
            tempBasal.endDate = span.end
            tempBasal.deliveredUnits = dose.deliveredUnits
                .map { self.adjustPumpedVolumeToU100(pumpedVolume: $0) as NSDecimalNumber }
            event.isMutable = dose.isMutable
