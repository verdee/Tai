...
@MainActor final class ReleaseNotesService: ObservableObject {
    ...
    /// Repository the notes are read from, set as `RELEASE_NOTES_REPO` in Config.xcconfig so a
    /// fork points at its own releases without this file being patched on every upstream merge.
    private static let repository: String = {
        let configured = Bundle.main.object(forInfoDictionaryKey: "ReleaseNotesRepository") as? String
